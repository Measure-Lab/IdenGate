...
WEIGHT_PTH = "/home/ubuntu/PycharmProjects/MIA/outputs/extral_experiments/baseline/1/cmanet_blood_dp_best.pth"
...
OUT_CSV = os.path.join(OUT_DIR, "translation_robustness.csv")

TRANSLATION_PIXELS = [50, 80, 110, 140, 170]

TRANSLATION_MODE = "random_xy"
...
class DermNPZTranslationDataset(Dataset):

    def __init__(self, images, labels, shift_px: int = 0, img_size=IMG_SIZE, mode="random_xy"):
        self.images = images
    ...
    def _sample_dxdy(self, p: int):
        if p <= 0:
            return 0, 0
        if self.mode == "fixed_right":
            return p, 0
        dx = random.randint(-p, p)
        dy = random.randint(-p, p)
        return dx, dy
    ...
    def __getitem__(self, i):
        ...
        if self.shift_px > 0:
            dx, dy = self._sample_dxdy(self.shift_px)
            pil = TF.affine(
                pil,
                angle=0.0,
                translate=[dx, dy],
...
def write_csv(path, rows):
    ...
    with open(path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["translation_shift_pixels", "acc", "auc_macro", "auc_weighted"])
        w.writerows(rows)
...
def main():
    ...
    print(f"[INFO] Translation mode: {TRANSLATION_MODE}")

    results = []
    for p in TRANSLATION_PIXELS:
        set_seed(42 + int(p))

        ds = DermNPZTranslationDataset(
            x_te, y_te, shift_px=p, img_size=IMG_SIZE, mode=TRANSLATION_MODE
        )
        loader = DataLoader(
        ...
        print(f"[Shift={p:02d}px]  Acc={acc:.2f}%  AUC(macro)={auc_macro:.4f}  AUC(weighted)={auc_weighted:.4f}")
        results.append([f"{p:d}", f"{acc:.4f}", f"{auc_macro:.6f}", f"{auc_weighted:.6f}"])
